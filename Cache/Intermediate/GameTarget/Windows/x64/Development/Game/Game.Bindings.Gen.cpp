...
public:
    ...
    DLLEXPORT static MObject* Get_jibPrefab(PlayerController* __obj)
    {
        MSVC_FUNC_EXPORT("Game.PlayerController::Internal_Get_jibPrefab")
        if (__obj == nullptr) DebugLog::ThrowNullReference();
        return __obj->_jibPrefab.GetManagedInstance();
    }

    DLLEXPORT static void Set_jibPrefab(PlayerController* __obj, Prefab* value)
    {
        MSVC_FUNC_EXPORT("Game.PlayerController::Internal_Set_jibPrefab")
        if (__obj == nullptr) DebugLog::ThrowNullReference();
        __obj->_jibPrefab = value;
    }
